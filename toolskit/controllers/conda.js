...
function runSnakefile(retour) {
    ...
    const maxConcurrentJobs = numCPUs - 2;
    ...
    var commands = [
        ...
        conda_path + ' init',
        conda_path + ' activate snakemake',
        '~/miniconda3/condabin/conda run -v -n snakemake snakemake -j'+maxConcurrentJobs,
    ]
...
const runMultiqc = (req, res) => {
    ...
    console.log(samples);
    runYamlFile(samples);
    genFile("./controllers/multiqc_snk/Snakefile", "./uploads/res/multiqc")
    runSnakefile(res);
}
...
const runQc = (req, res) => {
    ...
    samples += "parameter:\n";
    if(parameters.getForConda(req.body.parameter).trim_front1==0){
        samples += "    trim_front1 : \n";
    }else{
        samples += "    trim_front1 : --clip_R1 " + parameters.getForConda(req.body.parameter).trim_front1 + "\n";
    }
    if(parameters.getForConda(req.body.parameter).trim_front2==0){
        samples += "    trim_front2 : \n";
    }else{
        samples += "    trim_front2 : --clip_R2 " + parameters.getForConda(req.body.parameter).trim_front2 + "\n";
    }
    if(parameters.getForConda(req.body.parameter).trim_tail1==0){
        samples += "    trim_tail1 : \n";
    }else{
        samples += "    trim_tail1 : --three_prime_clip_R1 " + parameters.getForConda(req.body.parameter).trim_tail1 + "\n";
    }
    if(parameters.getForConda(req.body.parameter).trim_tail2==0){
        samples += "    trim_tail2 : \n";
    }else{
        samples += "    trim_tail2 : --three_prime_clip_R2 " + parameters.getForConda(req.body.parameter).trim_tail2 + "\n";
    }
    if(parameters.getForConda(req.body.parameter).n_base_limit==0){
        samples += "    n_base_limit : \n";
    }else{
        samples += "    n_base_limit : --max_n " + parameters.getForConda(req.body.parameter).n_base_limit + "\n";
    }
    if(parameters.getForConda(req.body.parameter).max_lg==0){
        samples += "    max_lg : \n";
    }else{
        samples += "    max_lg : --length " + parameters.getForConda(req.body.parameter).max_lg + "\n";
    }
    if(parameters.getForConda(req.body.parameter).max_ql==0){
        samples += "    max_ql : \n";
    }else{
        samples += "    max_ql : -q " + parameters.getForConda(req.body.parameter).max_ql + "\n";
    }
    samples += "ENVS:\n";
    samples += "    SNK_ENV : source activate snakemake\n";
    samples += "    home : " + require('path').resolve(__dirname, '..');
...
const runDbsub = (req, res) => {
    ...
    if(req.body.qualtiy==0 || req.body.qualtiy==''){
        samples += "    quality : \n";
    }else{
        samples += "    quality: -q " + req.body.qualtiy + "\n";
    }
    samples += "ENVS:\n";
    samples += "    SNK_ENV : source activate snakemake\n";
    samples += "    home : " + require('path').resolve(__dirname, '..');
...
const runGenCons2 = (req, res) => {
   // console.log(req.body)
    var dirPath = 'uploads/res/cons/references' + '|' + req.body.depth + '|' + req.body.quality + '|' + req.body.ploidy
  //  console.log(req.body,dirPath)
    fs.readdir(dirPath.split("|")[0], function (err, files) {
        ...
        if (err) {
            ...
            samples += "    ploidy: " + dirPath.split("|")[3] + "\n";
            samples += "samples : \n";
            for (i = 0; i < data_.length; i++) {
