...
@Component({
  selector: 'app-pathogene',
  templateUrl: './pathogene.component.html',
  styleUrls: ['./pathogene.component.scss']
})
export class PathogeneComponent implements OnInit {
      ...
      this.uploadService.upload(file, this.folder).subscribe(
        (event: any) => {
          if (event.type === HttpEventType.UploadProgress) {
            ...
            this.chargedTableList('ref_patho');
          }
        },
        (err: any) => {
  ...
  /*   link() {
      this.router.navigate(['/result', { id: 'ref_patho' }]);
    } */
  ...
  index() {
    ...
    if (tables.length == 0) { } else {
      this.showlog = true;
      this.click = true;
      this.loaded = true;
      this.logCmd = ''; this.logConsole = ''; this.logCode = '';
      this.condaService.indexp(tables, 'patho')
        .subscribe(
          response => {
            this.showlog = false
            this.color = 'white'
  ...
  check() {
    ...
    if (tables.length == 1) {
      ...
      this.loaded = true;
      this.logCmd = ''; this.logConsole = ''; this.logCode = '';
      this.condaService.checkg(tables, 'patho')
        .subscribe(
          response => {
            this.showlog = false
            this.color = 'white'
  ...
  delete() {
    ...
    this.uploadService.deleteFiles(tables, 'patho')
      .subscribe(
        response => {
